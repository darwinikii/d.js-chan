...
export async function executeUser (client, interaction, target) {
  await interaction.reply({ content: 'You runned user context example command with <@' + target.id + '>' })
}

export async function executeMessage (client, interaction, target) {
  await interaction.reply({ content: 'You run message context example command with :' + target.content })
}
